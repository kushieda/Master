...
/*   Updated: 2014/01/13 05:07:32 by jyim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_dda(t_env *e)
{
	...
	if (e->side == 0)
		e->perpwalldist = fabs(((double)e->mapx - e->rayposx
				+ (double)(1 - e->stepx) / 2) / e->raydirx);
	else
		e->perpwalldist = fabs(((double)e->mapy - e->rayposy
				+ (double)(1 - e->stepy) / 2) / e->raydiry);
}

static void	ft_initray2(t_env *e)
{
	if (e->raydirx < 0)
	{
		e->stepx = -1;
